...
<?
  ...
  if(isset($_SESSION["id"]))
  {
    ...
    if(!$db->planets_building_has($_SESSION['coords'],12,1)) //hat keine schiffsfabrik
    {
      $smarty->assign("menu","Produktion");
      $smarty->assign("you_need","Sie ben&ouml;tigen eine Schiffsfabrik");
  ...
  if(isset($_GET['a']))
    {
      ...
      if($_GET['a'] == "del")
      {
        $db->ships_build_delete($_GET['id']);
      }
      else if($_GET['a'] == "down")
      {
        $db->ships_build_down($_GET['id']);
      }
      else
      {
        ...
        if(isset($_POST['p'.$i]) && ($_POST['p'.$i] >0))
	{
	  ...
	  $db->ships_build($i,$_POST['p'.$i],$_SESSION['coords']);
	}
      }
    } 
    $db->reinit();
    $shiplist = $db->shiplist($_SESSION['coords'],$_SESSION['id']);
    $db->reinit();
    $buildlist = $db->ship_buildlist($_SESSION['coords']);
    ...
    $smarty->display("produktion.thtml");
  }
  else
  {
    session_destroy();
